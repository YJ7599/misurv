...
mirkat.bin <- function(beta.bin.out) {
  set.seed(487)
  out <- try(MiRKAT(y = as.numeric(beta.bin.out$bin.var)-1, X = NULL, Ks = beta.bin.out$Ks, out_type = "D", nperm = 3000), silent = TRUE)
...
mirkat.bin.plot1 <- function(out, beta.bin.out) {
  mod <- list()
  sub.tit <- list()
  print(length(beta.bin.out$Ds))
  for (i in 1:length(beta.bin.out$Ds)) {
    print(out$p_values)
    if (out$p_values[i] < 0.05) {
    ...
    mod[[i]] <- betadisper(as.dist(beta.bin.out$Ds[[i]]), beta.bin.out$bin.var)
  }
  return(list(mod = mod, sub.tit = sub.tit))
}

mirkat.bin.plot2 <- function(out, beta.bin.out, mod, sub.tit) {
  par(mfrow = c(3,2))
  for (i in 1:length(beta.bin.out$Ds)) {
...
mirkat.bin.cov <- function(beta.bin.cov.out) {
  set.seed(487)
  out <- try(MiRKAT(y = as.numeric(beta.bin.cov.out$bin.var)-1, X = as.matrix(beta.bin.cov.out$cov.var), Ks = beta.bin.cov.out$Ks, out_type = "D", nperm = 1000), silent = TRUE)
