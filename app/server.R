    ...
    withProgress(
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        if (input$chooseMethod_taxa == "Welch t-test") {
          ...
          output$airpods = renderUI({
            tagList(
              tabBox(title = strong("Box Plot", style = "color:black"), width = NULL,
                     tabPanel("Phylum", align = "center",
                              plotOutput("rank_1", height = height[1]*250, width = 750),
                     )
                     ,
                     tabPanel("Class", align = "center",
                              plotOutput("rank_2", height = height[2]*250, width = 750),
                     )
                     ,tabPanel("Order", align = "center",
                               plotOutput("rank_3", height = height[3]*250, width = 750),
                     )
                     ,tabPanel("Family", align = "center",
                               plotOutput("rank_4", height = height[4]*250, width = 750),
                     )
                     ,tabPanel("Genus", align = "center",
                               plotOutput("rank_5", height = height[5]*250, width = 750),
                     )
                     ,tabPanel("Species", align = "center",
                               plotOutput("rank_6", height = height[6]*250, width = 750),
                     )
              )
            )
          })
          ...
          first.taxa.sig <- eraizza(taxa.outputs$DAoutput, chooseData$NAadded$tax.tab, "twopi", include)
        ...
        output$gdownload <- downloadHandler(
          ...
          content = function(file) {
            htmlwidgets::saveWidget(as_widget(eraizza(taxa.outputs$DAoutput, chooseData$NAadded$tax.tab, "twopi", include)$flow.text), file)
          }
        )
  ...
  observeEvent(input$runbtn_CoxT, {
    ...
    withProgress(
      ...
      detail = 'This may take a while...', value = 0, {
        ...
        surv.tree.data <- surv.taxa.forest.plot.pages1(taxa.cox.out, taxa.names.out.surv, report.type = "HR", species.include = include, mult.test.cor = TRUE)
        ...
        if (duplicate.texts > 0) {
          ...
          lapply(1:nrow, function(j) {
            output[[paste0("duplicates_surv", j)]] <- renderPlot({
              duplicate.surv(duplicate.taxa, taxon.inplot, chooseData$taxa.names.out$duplicates)
            })
          })
        ...
        output$gdownload <- downloadHandler(
          ...
          content = function(file) {
            htmlwidgets::saveWidget(as_widget(taxa.sig.dend.surv(taxa.cox.out, chooseData$NAadded$tax.tab, "twopi", include)), file)
          }
        )
